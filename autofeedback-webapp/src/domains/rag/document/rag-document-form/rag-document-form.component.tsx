...
import {
    Button, Checkbox, FormControl, FormControlLabel, InputLabel, List, ListItem, ListItemButton, ListItemText,
    TextField,
    Typography
} from "@mui/material";
...
export default function RagDocumentFormComponent(props: PromptGroupFormProps) {
    ...
    const externallyManagedInput = useInputValue<boolean>(props.externallyManged ? props.externallyManged : false, {required: false});
    const textInput = useInputValue<string>(props.textInit, {required: true});
    const categoryInput = useInputValue<string>(props.categoryInit, {required: false});
    const languageInput = useInputValue<string>(props.languageInit, {required: false});
    const topicInput = useInputValue<string>(props.topicInit, {required: false});
    const typeInput = useInputValue<string>(props.typeInit, {required: false});
    const constructsInput = useInputValue<string[]>(props.constructsInit, {required: false});
    ...
    const saveRagDocument = () => {
        props.save(
            new RagDocumentUpdate(
                ...
                externallyManagedInput.valueOrThrow(),
                new RagDocumentMetadataUpdate(
                    textInput.valueOrThrow(),
                    categoryInput.valueOrUndefined(),
                    languageInput.valueOrUndefined(),
                    topicInput.valueOrUndefined(),
                    typeInput.valueOrUndefined(),
                    constructsInput.valueOrUndefined()
                )
            )
        )
    }
        ...
        <div className={'rag-document-form-container'}>
            <FormControl style={{width: 400}}>
                <FormControlLabel control={
                    <Checkbox value={externallyManagedInput.value}
                              onChange={externallyManagedInput.handleChange}
                              required={externallyManagedInput.options.required}
                    />
